# gw 仕様書

**バージョン:** 0.1.0

`gw` は git worktree の薄いラッパー CLI ツールである。パス自動計算とフック実行を核とする。

**出力規約:** stdout にはデータ（パス等）のみを出力し、stderr にログ・フック出力・エラーを出力する。

---

## 1. コマンド

### 1.1 `gw add <branch> [--from <ref>]`

worktree を作成し、作成先パスを stdout に出力する。

#### 引数・オプション

| 引数/オプション | 必須 | 説明 |
|---|---|---|
| `<branch>` | はい | 作成するブランチ名 |
| `--from <ref>` | いいえ | 指定した ref からブランチを作成。省略時はデフォルトブランチ |

#### 処理フロー

1. リポジトリルートを検出
2. worktree パスを計算（[2. パス計算](#2-パス計算)参照）
3. `git worktree add <path> -b <branch> [<ref>]` を実行
4. `post-add` フックを実行（[3. フックシステム](#3-フックシステム)参照）
5. 作成先パスを stdout に出力

#### 出力

- **stdout**: worktree の絶対パス
- **stderr**: ログメッセージ

---

### 1.2 `gw rm <identifier> [--force]`

worktree を削除する。

#### 引数・オプション

| 引数/オプション | 必須 | 説明 |
|---|---|---|
| `<identifier>` | はい | 削除対象の worktree（[4. identifier 解決](#4-identifier-解決)参照） |
| `--force` | いいえ | 未コミット変更がある worktree も強制削除。`pre-rm` フック失敗時も続行 |

#### 処理フロー

1. identifier から worktree パスを解決
2. `pre-rm` フックを worktree ディレクトリ内で実行
   - 失敗かつ `--force` なし: エラー終了
   - 失敗かつ `--force` あり: 警告を出力して続行
3. `git worktree remove [--force] <path>` を実行
4. `post-rm` フックをリポジトリルートで実行

---

### 1.3 `gw go <identifier>`

worktree のパスを stdout に出力する。`cd "$(gw go <id>)"` のようにシェル連携で使用する。

#### 引数

| 引数 | 必須 | 説明 |
|---|---|---|
| `<identifier>` | はい | 対象の worktree（[4. identifier 解決](#4-identifier-解決)参照） |

#### 出力

- **stdout**: worktree の絶対パス
- **stderr**: ログメッセージ

---

### 1.4 `gw version`

バージョン情報を出力する。

```
gw version <VERSION>
```

---

## 2. パス計算

### 2.1 ベースディレクトリ

worktree を格納する親ディレクトリ。

| 優先順位 | ソース | 値 |
|---|---|---|
| 1 | 環境変数 `GW_WORKTREES_DIR` | 指定されたパス |
| 2 | デフォルト | `../<repo-name>-worktrees/` （リポジトリの親ディレクトリ基準） |

### 2.2 ブランチ名サニタイズ

ブランチ名をファイルシステム上で安全なフォルダ名に変換する。

**変換ルール:**
1. `/` をハイフン（`-`）に置換
2. 先頭・末尾のハイフンを除去

**変換例:**

| 入力 | 出力 |
|---|---|
| `feature/user-auth` | `feature-user-auth` |
| `feature/auth/login` | `feature-auth-login` |

**バリデーション:** サニタイズ結果が空文字列、`.`、`..` の場合はエラー。

### 2.3 最終パス

```
<base_dir>/<sanitized-branch>
```

---

## 3. フックシステム

`.gw/hooks/` ディレクトリに実行可能ファイルを配置する（git hooks パターン）。

### 3.1 フックフェーズ

| フック名 | トリガー | 実行場所 |
|---|---|---|
| `post-add` | worktree 作成後 | worktree ディレクトリ |
| `pre-rm` | worktree 削除前 | worktree ディレクトリ |
| `post-rm` | worktree 削除後 | リポジトリルート |

### 3.2 フック環境変数

フック実行時に以下の環境変数がエクスポートされる:

| 変数 | 説明 |
|---|---|
| `GW_REPO_ROOT` | メインリポジトリルートの絶対パス |
| `GW_WORKTREE_PATH` | worktree の絶対パス |
| `GW_BRANCH` | ブランチ名 |

### 3.3 フック実行ルール

- フックファイルが存在しない場合: 何もせず成功扱い
- フックファイルが存在するが実行権限がない場合: エラー
- フックの stdout/stderr は親プロセスの stderr に流す
- 各フックは独立したサブプロセスで実行する

### 3.4 フック失敗時の動作

| フック | 失敗時の動作 | `--force` 時 |
|---|---|---|
| `post-add` | 警告のみ（worktree は作成済み） | - |
| `pre-rm` | **削除を中止** | 警告して削除を続行 |
| `post-rm` | 警告のみ（worktree は削除済み） | - |

---

## 4. identifier 解決

`gw go` / `gw rm` で使用する identifier からworktree パスを逆引きする。

**解決順序（最初にマッチしたものを返す）:**

1. **サニタイズ名によるパスマッチ**: `<base_dir>/<sanitize(identifier)>` がディレクトリとして存在すればそのパスを返す
2. **ブランチ名によるスキャン**: ベースディレクトリ内のディレクトリを走査し、各ディレクトリの `git worktree` のブランチ名が identifier と一致するかチェック
3. **すべて失敗**: エラー

---

## 5. エラー処理・終了コード

### 5.1 終了コード

| コード | 意味 |
|---|---|
| `0` | 成功 |
| `1` | エラー |

### 5.2 エラーメッセージ

エラーメッセージは stderr に出力する。

### 5.3 主要なエラーケース

| 状況 | 終了コード |
|---|---|
| git リポジトリ外での実行 | 1 |
| 不明なコマンド | 1 |
| worktree が見つからない（identifier 解決失敗） | 1 |
| worktree が既に存在する | 1 |
| 無効なブランチ名（サニタイズ後のバリデーション失敗） | 1 |
| git コマンドの失敗 | 1 |

---

## 6. 将来の拡張（初回スコープ外）

- `gw list`: worktree 一覧表示
- git worktree サブコマンドのパススルー
- シェル補完
